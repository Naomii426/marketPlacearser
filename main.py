...
class WbParser:
    ...
    def __create_csv(self):
        with open('marketPlaces.csv', mode='w', newline='', encoding='utf-8') as file:
            ...
            writer.writerow(['Наименование товара', 'Цена', 'Рейтинг', 'Ссылка', 'Продавец', 'Продавец_URL'])
    ...
    def __save_csv(self):
        with open('marketPlaces.csv', mode='w', newline='', encoding='utf-8') as file:
            ...
            for product in self.items.products:
                writer.writerow([product.name, product.price, product.rating, product.url, product.name_of_seller, product.link_on_seller])
    ...
    async def __get_info(self, url: str):
        ...
        try:
            ...
            name_of_seller = await self.page2.inner_text(".seller-info__name")
            url_on_seller = await self.page2.query_selector(".seller-info__name")
            link_on_seller = await url_on_seller.get_attribute("href")
            ...
            name_of_seller = await self.page2.inner_text(".seller-info__name")
            url_on_seller = await self.page2.query_selector(".seller-info__name")
            link_on_seller = "https://www.wildberries.ru" + await url_on_seller.get_attribute("href")
            item = Item(url, name_of_product, price_of_product, rate_of_product, name_of_seller, link_on_seller)
            self.items.products.append(item)
            self.__save_csv()
            print(
